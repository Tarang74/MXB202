...
Given the scalar field \(\phi : D \subset \R^n \to \R\) and a curve
\(\mathscr{C}\) parametrised by \(\symbf{r}\left( t \right)\) with \(t
\in \interval{a}{b}\), where \(\symbf{r}\left( a \right) =
\symbf{r}\left( b \right)\), the line integral of \(\phi\) over
...
whose boundary \(\partial \Omega\) is a finite collection of closed, piecewise-smooth
    orientable surface. Let \(\symbf{F} : \Omega \to \R^n\) be a
...
\end{equation*}
\begin{corollary}[Divergence Theorem in 2D]
    ...
    \end{equation*}
\end{corollary}
\begin{corollary}[Curl Theorem in 2D]
